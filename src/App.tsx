...
const DEFAULT_RESOLUTION: Resolution = [1280, 720];
...
export default function App() {
  ...
  const [screenAllowed, setScreenAllowed] = useState<boolean | null>(null);
  ...
  const toggleRecording = useCallback(
    function toggleRecordingCb() {
      if (recorder) {
        ...
        if (microphoneAllowed && microphoneAudio) {
    ...
    [recorder, setRecorder, microphoneAllowed, microphoneAudioRef, canvasRef, setDownloadHref]
  );

  const requestScreenAccess = useCallback(
    function requestScreenAccessCb() {
      navigator.mediaDevices
        .getDisplayMedia({ video: true, audio: false })
        .then((stream) => {
          setScreenAllowed(true);
          stream.addEventListener('inactive', function screenStreamEnded() {
            setScreenAllowed(null);
          });
    ...
    [screenVideoRef, setScreenAllowed]
  );

  const requestCameraAccess = useCallback(
    function requestCameraAccessCb() {
      navigator.mediaDevices
    ...
    [cameraVideoRef, setCameraAllowed]
  );

  const requestMicrophoneAccess = useCallback(
    function requestMicrophoneAccessCb() {
      navigator.mediaDevices
        .getUserMedia({
          video: false,
        ...
        .then((stream) => {
          ...
          stream.addEventListener('inactive', function microphoneStreamEnded() {
            setMicrophoneAllowed(null);

            if (recorder) {
              stream.getAudioTracks().forEach(recorder.stream.removeTrack.bind(recorder.stream));
            }
          });
          ...
          if (audioEl) {
            ...
            audioEl.play();
            if (recorder) {
              stream.getAudioTracks().forEach(recorder.stream.addTrack.bind(recorder.stream));
            }
          }
        })
        .catch((error) => {
    ...
    [microphoneAudioRef, setMicrophoneAllowed, recorder]
  );
    ...
    <Fragment>
      <header className="main-header">
        ...
        <div className="toolbar">
          ...
          <button
            disabled={/* !!recorder || */ screenAllowed !== null}
            onClick={requestScreenAccess}>
            Include screen
          </button>
          <button
            disabled={/* !!recorder || */ cameraAllowed !== null}
            onClick={requestCameraAccess}>
            Include camera
          </button>
          <button
            disabled={!!recorder || microphoneAllowed !== null}
      ...
      <footer className="main-footer">
        ...
        {<PermissionAlert permission={microphoneAllowed}>Microphone</PermissionAlert>}
        {<PermissionAlert permission={cameraAllowed}>Camera</PermissionAlert>}
        {<PermissionAlert permission={screenAllowed}>Screen</PermissionAlert>}
        <img
          ref={imagePatternRef}
